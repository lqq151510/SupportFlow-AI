...
class ToolExecutionServiceTest {
    ...
    void turnsHighRiskToolsIntoApprovalRequests() {
        CommerceSupportToolService commerce = mock(CommerceSupportToolService.class);
        ManageApprovalService approvals = mock(ManageApprovalService.class);
        when(approvals.create(any(), any(), any(), any(), any())).thenReturn(new Approval(9L, "refund.request", "refund", ApprovalStatus.PENDING, Instant.now()));
        ...
        verify(approvals).create(1L, 2L, "refund.request", "refund.request for order DEMO-001", null);
    }
}
