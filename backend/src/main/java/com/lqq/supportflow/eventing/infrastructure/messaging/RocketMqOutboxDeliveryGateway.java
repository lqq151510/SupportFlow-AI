...
public class RocketMqOutboxDeliveryGateway implements OutboxDeliveryGateway {
    ...
    @Override public void publish(OutboxEvent event) { try { producer.send(new Message(topic, json.writeValueAsBytes(new RocketMqEnvelope(event.id(), event.tenantId(), event.eventType(), event.payload())))); } catch (Exception exception) { throw new IllegalStateException("RocketMQ publish failed", exception); } }
