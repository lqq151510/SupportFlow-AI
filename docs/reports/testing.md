...
- 后端：`mvn -q verify` 通过，152 项测试无失败、错误或跳过；新增工单正式回复覆盖“坐席已认领才可发送、同一幂等键只写一条、消费者仅能读取自己的会话”。
- 前端：Vitest 13/13、`vue-tsc -b && vite build` 通过。
- 浏览器：完整 Compose 上 `npm run test:e2e` 2/2 通过。消费者转人工后，坐席完成认领、内部备注、正式回复；消费者刷新会话后可见该回复，再完成解决与关闭。
- 运行环境：`supportflow-java` Compose 后端健康端点返回 `UP`；MinIO 使用 19000/19001 宿主机映射，启动期密钥为本次进程临时随机生成，未写入仓库或配置文件。
- 稳定性：两条 E2E 场景共享本机 Compose 中间件，固定为 Playwright 单 worker 执行，避免并行测试在 30 秒上限内竞争资源。

### JaCoCo 门禁修复复验（2026-09-18）

- 补充工单正式回复的领取校验、关闭态拒绝、幂等冲突和消费者会话边界测试；同时覆盖客服工具的未知/缺参分支、成员禁用令牌撤销与退款资格边界。
- 执行 `mvn -q -f backend/pom.xml clean verify`：168 项测试，失败和错误均为 0；本机 Docker daemon 未运行，因此 3 项 Testcontainers 测试按条件跳过。
- JaCoCo：行覆盖率 89.78%（1967/2191），分支覆盖率 75.05%（725/966），满足 Maven 强制门禁（85% / 75%）。
- 回归目的：修复 CI 上新增客服回复流程使分支覆盖率从 74% 低于阈值而导致的门禁失败；远端 CI 仍会执行完整容器路径。
