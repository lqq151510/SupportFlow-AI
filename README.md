...
账号、密码和 JWT 密钥仅适用于本地开发，禁止把真实生产密钥写入 `.env.example` 或提交到仓库；完整应用启动前需在 `.env` 中设置 `SUPPORTFLOW_JWT_SECRET_BASE64` 和 `MODEL_SECRET_MASTER_KEY`，后者可用 `openssl rand -base64 32` 生成。Compose 默认使用租户已保存的真实模型配置；只有显式设置 `SUPPORTFLOW_MODEL_MOCK_ENABLED=true` 时才启用确定性 Mock，避免把演示结果误认为真实模型调用。
